    ...
    <div className={cn("space-y-4", className)}>
      <div className="relative overflow-hidden rounded-2xl glass sm:rounded-3xl">
        <img
          ...
          className="mx-auto block w-full max-h-[min(78vh,720px)] object-contain"
          decoding="async"
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 [-webkit-overflow-scrolling:touch]">
          {list.map((src, i) => (
            <button
              ...
              onClick={() => setActive(i)}
              className={cn(
                "relative h-16 w-16 shrink-0 overflow-hidden rounded-xl border-2 bg-muted/40 transition-all sm:h-20 sm:w-20",
                active === i
